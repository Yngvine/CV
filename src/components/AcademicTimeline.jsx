...
const StyledAcademicTimeline = styled.section`
  p {
    font-size: .75rem;
  }
  .img {
...
const AcademicTimeline = () => {
  ...
  useEffect(() => {
    setEducation(Data.education.map((education) => ({
      level: education.level,
      icon: <Icon icon={education.institution_logo} className="display-4" />,
      institution: education.institution,
      dates: education.start_date ? `${education.start_date} - ${education.end_date}` : `${education.date}`,
  ...
  useLayoutEffect(() => {
    ...
    if (els.length === education.length) {
  ...
  return (
    <Element name={"Education"} id="education">
      <StyledAcademicTimeline className="section" 
        style={{ "--svg-max-width": maxWidth ? `${maxWidth}px` : undefined }}>
        <Container>
          <Container className="d-flex justify-content-center">
            <Title size={"h2"} text={"Academic Timeline"} />
          </Container>
          <Row className="mt-5">
